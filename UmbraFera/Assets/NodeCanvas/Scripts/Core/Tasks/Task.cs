...
namespace NodeCanvas{
	...
	abstract public class Task : MonoBehaviour{
		...
		[Serializable]
		///A special BBValue for the task agent
		class TaskAgent : BBValue{
		...
		public void SetOwnerDefaults(ITaskDefaults newOwner){
			...
			UpdateBBFields(newOwner.blackboard);
		}
		...
		private void CheckNullBBFields(){
			BBValue.CheckNullBBFields(this);
		}
		...
		private void AgentField(){
			...
			if (!taskAgent.useBlackboard){
				...
				if (taskAgent.bb){
					...
					var dataNames = taskAgent.bb.RetrieveDataNames(taskAgent.dataType).ToList();
					if (dataNames.Contains(taskAgent.dataName) || string.IsNullOrEmpty(taskAgent.dataName) ){
						...
						taskAgent.dataName = EditorGUILayout.TextField("Override (" + EditorUtils.TypeName(agentType) + ")", taskAgent.dataName);
					}
