...
namespace NodeCanvas{
	...
	static public class EditorUtils{
		...
		public static object GenericField(string name, object value, System.Type t, MemberInfo member = null){
			...
			if (member != null && member.GetCustomAttributes(typeof(RequiredFieldAttribute), true).FirstOrDefault() as RequiredFieldAttribute != null){
				if (t == typeof(string) && string.IsNullOrEmpty((string)value) ||
					typeof(BBValue).IsAssignableFrom(t) && (value as BBValue).isNull ||
			...
			if (t == typeof(string)){
				...
				if (member != null){
					...
					if (areaAtt != null){
						...
						return EditorGUILayout.TextField((string)value, GUILayout.Height(areaAtt.height));
					}
				}
			...
			if (typeof(BBValue).IsAssignableFrom(t)){
				...
				return BBValueField(name, (BBValue)value, member);
			}
			...
			GUILayout.Label("Type '" + TypeName(t) + "' not supported for Inspection");
			return value;
		}
		...
		public static BBValue BBValueField(string prefix, BBValue bbValue, MemberInfo member = null){
			...
			if (!bbValue.blackboardOnly && !bbValue.useBlackboard){
				...
				field.SetValue(bbValue, GenericField(prefix, field.GetValue(bbValue), field.FieldType, member) );
			...
			if (bbValue.bb && bbValue.useBlackboard && !bbValue.blackboardOnly && string.IsNullOrEmpty(bbValue.dataName)){	
			...
			if (member != null && bbValue.GetType() == typeof(BBGameObject)){
				var rc = member.GetCustomAttributes(typeof(RequiresComponentAttribute), true).FirstOrDefault() as RequiresComponentAttribute;
				if (rc != null && typeof(Component).IsAssignableFrom(rc.type) && !bbValue.isNull){
					if ( (bbValue as BBGameObject).value.GetComponent(rc.type) == null ){
		...
		public static void ShowComponentSelectionButton(GameObject target, Type baseType, Action<Component> callback, bool hideComponent = true){

			GUILayout.BeginHorizontal();
			GUI.backgroundColor = lightBlue;

			if (GUILayout.Button("Add " + TypeName(baseType))){
